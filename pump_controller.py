...
def customShadowCallback_Get(payload, responseStatus, token):
	...
	reported_humidity = payloadDict["state"]["reported"]["humidity_percentages"][0]
	desired_humidity = payloadDict["state"]["desired"]["humidity_percentages"][0]
	print "REPORTED HUMIDITY: "
	print reported_humidity
	if reported_humidity < desired_humidity:
...
if useWebsocket:
	myAWSIoTMQTTShadowClient = AWSIoTMQTTShadowClient("pump", useWebsocket=True)
	myAWSIoTMQTTShadowClient.configureEndpoint(host, 443)
	myAWSIoTMQTTShadowClient.configureCredentials(rootCAPath)
else:
	myAWSIoTMQTTShadowClient = AWSIoTMQTTShadowClient("pump")
...
Bot = myAWSIoTMQTTShadowClient.createShadowHandlerWithName("humidity_sensor_1", True)
...
#p = GPIO.PWM(PUMP_PIN, 100)  # channel=12 frequency=50Hz
#p.start(0)
